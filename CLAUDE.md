...
Nacho can hand Claude medical test results (blood work, DEXA scans, fitness tests, doctor reports). Claude stores these in `coach.db` via the import script and tracks them over time.

**How to import:**
```
python scripts/import_medical.py --date 2026-04-11 --category blood_work --name "Testosterone" --value 650 --unit "ng/dL"
```
Or pass a file:
```
...
**What to track**: testosterone, cortisol, HbA1c, glucose, lipid panel, DEXA body fat %, VO2max tests, blood pressure, any doctor findings.

**Claude's role with medical data:**
- Never diagnose or give medical advice
- Flag values outside reference range as "worth discussing with your doctor"
- Track trends over time (e.g., "Testosterone was 650 in April, up from 580 in January")
- Connect to training data where relevant (e.g., "Low testosterone correlates with the training block where recovery was poor")
- BRIEFING.md will include a Medical section with latest values per test
...
| scripts/import_medical.py | Import medical results into DB | When Nacho shares test results |
